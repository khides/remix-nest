...
const schemaSourceFile = project.addSourceFileAtPath(
  "./app/supabase/__generated__/schema.ts"
); // `schema.ts`のパスを指定
// 現在のファイルの絶対パスを取得
...
function typeToInterface(type: Type, indent = 2): string {
  ...
  if (
    type.getText() ===
    `import("${__dirname}/app/supabase/__generated__/schema").Json[]`
  ) {
    return "Json[]";
  }
...
// 出力先ディレクトリとファイルパス
const outputPath = "./app/supabase/__generated__/supabase.interface.d.ts";
