...
>(
  (
    { jsDate, onJsDateChange, showClearButton = true, onClear, ...props },
    ref
  ) => {
    ...
    return (
      <div
        ...
        className={cn(
          ...
          "flex items-center rounded-md border ring-offset-background"
        )}
      >
        ...
        <div className={cn("-ml-2 mr-3 h-5 w-5", !showClearButton && "hidden")}>
          <X
            className={cn(
              "h-4 w-4 cursor-pointer text-primary/30",
              !jsDatetime && "hidden"
            )}
            onClick={() => {
              setJsDatetime(null);
